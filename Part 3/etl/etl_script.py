...
def extract_orders_from_cassandra():
    """Extract all orders from Cassandra orders_by_customer table"""
    hosts = os.environ.get("CASSANDRA_HOSTS", "cassandra-node1").split(",")
    keyspace = os.environ.get("CASSANDRA_KEYSPACE", "techretail_orders")
    ...
    print(f"   Extracted {len(orders)} orders from Cassandra ({keyspace})")
    cluster.shutdown()
    return orders
