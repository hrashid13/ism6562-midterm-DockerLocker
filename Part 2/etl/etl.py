...
CHANGES FROM PART 1:
  - Replaced single get_connection("SALES") with TWO shard connections:
      get_connection("SALES_SHARD_SE")  ->  FL, GA, TN customers
      get_connection("SALES_SHARD_NE")  ->  NC customers
  - extract_sales() now accepts a conn and a label (for logging)
  - run_etl() calls extract_sales() on each shard and merges:
      * customers: combined from both shards
      * orders:    combined from both shards
      * products:  taken from SE shard only (reference table, identical on both)

Phases:
  1. EXTRACT  - Read rows from BOTH sales shards and hr-db
...
def extract_sales(conn, shard_label="shard"):
    ...
    """)
    orders = cur.fetchall()
    print(f"  [{shard_label}] Extracted {len(orders)} orders")
...
def run_etl():
    ...
    # --- EXTRACT ---
    print("\n[EXTRACT] Reading from both sales shards...")

    # --- FIX: connect to SE shard ---
    se_conn = get_connection("SALES_SHARD_SE")
    se_customers, products, se_orders = extract_sales(se_conn, shard_label="SE")
    se_conn.close()

    # --- FIX: connect to NE shard ---
    ne_conn = get_connection("SALES_SHARD_NE")
    ne_customers, _ne_products, ne_orders = extract_sales(ne_conn, shard_label="NE")
    ne_conn.close()

    # --- FIX: merge results ---
    # Products are reference data — identical on both shards, use SE only to avoid duplicates
    customers = se_customers + ne_customers
    orders    = se_orders    + ne_orders
    print(f"\n  Merged totals -> {len(customers)} customers, "
